...
class ExpectedSARSA:
    ...
    def learn(self, s, a, r, s_):
        ...
        if s_ != 'terminal':
            ...
            distribution = self.policy(s_)
            expected = sum([ distribution[i] * self.q_table.loc[s_,self.actions[i]] for i in range(len(self.actions))])
            q_delta = r + self.gamma * expected - self.q_table.loc[s, a]
